...
void update_arround(goban &gboard, const int16 &size, const int16 &x, const int16 &y, const cell::direction &dr, const int16 &sign)
{
    ...
    auto check_dr = [size, sign, dr](goban &gboard, bool &check, int16 &nsign, const int16 &x, const int16 &y){
    ...
    for (int16 i = 1; i < 5; i++) {
        ...
        if (check1) {
            check_dr(gboard, check1, sign1, x+ dr_x*i, y+ dr_y*i);
        }

        check2 = check2 && x- dr_x*i >= 0 && y- dr_y*i >= 0 && y- dr_y*i < size;
        if (check2) {
            check_dr(gboard, check2, sign2, x- dr_x*i, y- dr_y*i);
        }
