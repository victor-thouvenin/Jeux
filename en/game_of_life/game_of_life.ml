...
let arg = if (Array.length Sys.argv) < 2 then "" else Sys.argv.(1);;
let strm = try open_in arg with e -> print_string (Printexc.to_string e); Printf.printf "\n"; exit 1
let tab = strmToTab strm;;
close_in strm;;
if check_size tab then () else (Printf.printf "error: wrong format\n"; exit 1)
